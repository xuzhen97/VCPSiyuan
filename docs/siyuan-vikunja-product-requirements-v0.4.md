...
图片附件在行内提供「预览附件」，点击后在模态浮层中显示，Esc、点击遮罩或「关闭」均可退出。

- 已上传的图片：通过 `vikunja.attachments.download` 请求 `preview_size=xl`（800 px 缩放预览），沿用 Vikunja 自身的预览缓存；首次打开约 1 s，原图 1.29 MB → 预览 678 KB。
- 尚未上传的草稿：直接用内存中的 `File` 生成 object URL，不产生网络请求，显示原分辨率。
- 非图片附件不显示该入口；宿主未提供查看器时也不渲染按钮，避免出现无效操作。
- 对象 URL 在浮层关闭时释放。
...
草稿中的图片在保存前即可预览（读取内存中的文件），避免先保存再发现选错文件。
...
项目管理与标签管理是两个**独立页面**，各有自己的入口，互不嵌套、互不带跳转链接：两者的权限语义（项目删除需 admin，标签按可写判定）与删除影响范围不同，任一被藏在另一页里都会降低可发现性。
...
表头为标题与关闭按钮；工具栏为搜索框与主操作同排；列表行为色点、名称、元信息（已归档）与右对齐操作。

点击删除后，列表被**删除确认页**替换（而不是在列表下方追加面板），确认页包含目标名称、影响摘要、需手写目标标题的输入框，以及取消返回：
...
影响预览按需获取：仅在用户点击某行的删除后请求 `vikunja.projects.deleteImpact`，不再为列表第一项预取。
...
权限：编辑需 write 以上，删除需 admin 以上；不可用的操作保持可见但置灰。
...
页面结构与项目管理一致（表头 + 搜索/新建同排 + 行内色点与用量 + 删除确认替换列表）。
...
说明：Vikunja 的 `GET /labels` 不返回 `max_permission`（仅单条读取返回），而标签属于当前用户且写入由服务端校验，因此字段缺失按可写处理；否则列表每一行都会被判为不可写，标签页将退化为只读。
