...
VCPSiyuan 将思源笔记连接到 **Vikunja API v2**。任务写入要求服务端主版本为 2 且版本不低于 **v2.5.0**。v2.5.0 是最低验证基线，不代表所有更高 v2 版本均已实测。插件提供原生 `RightTop` 任务工作台，包含「收件箱、全部任务、项目标签」页签、逐级展开的子任务、任务详情及创建/关联/完成/解除子任务操作。

## 支持边界

- 任务写入要求 Vikunja API **v2**、服务端主版本 **2** 且版本 **≥ v2.5.0**。设置中填写实例 **Origin**（例如 `https://tasks.example.com`），插件内部固定追加 `/api/v2`。**v2.5.0 是最低验证基线，不表示所有更高 v2 版本都已测试。** 新配置不支持 API v1 或自定义 API 路径；旧的 `/api/v1` 设置只在迁移时规范化。
...
- 任务详情展示子任务关系。Dock 保留服务端平铺筛选与分页，只对已加载且命中筛选的任务组树，展开不会改变服务端总数或分页边界。解除关系只删除指定关系边，不删除任务。新建子任务默认使用父任务项目，但保存前可更换。如果创建成功而关联失败，插件保留已创建任务 ID，恢复操作只重试关联，不会重复创建。
- 任务写入使用乐观版本检查；发生冲突时会阻止覆盖更新的远端任务并提示处理。v2.5.0 验证基线的 `/info` 声明 `concurrent_writes: false`，且 `updated` 及其派生的 ETag 只有**秒级精度**，远端任务至少一秒前被修改时，本地保存可检测到过期；同一秒内的两次写入连服务端自身也无法区分。
